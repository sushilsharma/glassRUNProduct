angular.module("glassRUNProduct").controller('OrderGridController', function ($scope, $rootScope, $sessionStorage, $state, $filter, $ionicModal, $location, pluginsService, GrRequestService) {
    ...
    $scope.TransporterFilter = function (element) {


        var TransporterGridList = [];
        if ($scope.TransporterList.length > 0) {
            for (var i = 0; i < $scope.TransporterList.length; i++) {
                TransporterGridList.push($scope.TransporterList[i].TransporterId);
            }
        }
        element.kendoDropDownList({
            dataSource: TransporterGridList,
            optionLabel: "Carrier",
            valuePrimitive: true,
        });
    }
    ...
    $scope.OrderDetailsGrid =
        {
                        ...
                        var gridrequestData =
                            {
                                ...
                                RoleId: $rootScope.RoleId,
                                UserId: $rootScope.UserId,
                                PageName: $rootScope.resData.res_OMOrderPage_OrderList,
                                ObjectId: 16,
                                ObjectName: 'Order',
                                ControllerName: page
                            };
                                ...
                                if (resoponsedata != null) {
                                    ...
                                    if (resoponsedata.Json !== undefined) {
                                        ...
                                        for (var i = 0; i < ListData.length; i++) {
                                            ...
                                            ListData[i].IsAvailableStock = true;

                                            ListData[i].OrderGUID = generateGUID();
                                            ListData[i].IsEditOrderStatusConfiguration = false;
                                            ...
                                            if (orderList.length > 0) {
                                                ...
                                                if ($scope.SelectedOrderId == orderList[0].OrderId) {
                                                    if (($scope.SelectedDeliveryPersonnelId !== "" && $scope.SelectedDeliveryPersonnelId !== undefined) && ($scope.SelectedPlateNumber !== "" && $scope.SelectedPlateNumber !== undefined)) {
                                                        orderList[0].DeliveryPersonnelId = $scope.SelectedDeliveryPersonnelId;
                                                        orderList[0].PlateNumber = $scope.SelectedPlateNumber;

                                                    }
                                                    else {
                                                        orderList[0].PlateNumber = $scope.SelectedPlateNumber;
                                                        orderList[0].DeliveryPersonnelId = "";
                                                    }
                                                }
            ...
            columns: [
...

                {
                    ...
                    template: "<select class=\"form-control\" style=\"margin:0px !important\"  ng-model=\"dataItem.StockLocationId\" ng-change=\"getSelectedBranchPlantvalue(dataItem.StockLocationId,#=OrderId#)\" ng-options=\"obj.DeliveryLocationId as obj.DeliveryLocationName for obj in bindAllBranchPlant\"><option value=\"\">Select</option></select>",
                ...
                {
                    field: "TransporterId",
                    template: "<select class=\"form-control\" style=\"margin:0px !important\"  ng-model=\"dataItem.TransporterId\" ng-change=\"getSelectedTransporatorvalue(dataItem.TransporterId,#=OrderId#)\" ng-options=\"obj.TransporterId as obj.TransporterName for obj in TransporterList\"><option value=\"\">Select</option></select>",
                    title: $rootScope.resData.res_GridColumn_TransporterId,
                    width: "250px",
                    filterable: {
                        cell: { showOperators: false, template: $scope.TransporterFilter },
                        ui: $scope.TransporterFilter, mode: "row", extra: false, operators: {
                            string: {
                                eq: "Is equal to",
                            }
                        }
                ...
                //{
                //    field: "RequestDate",
                //    template: "<span Id=\"{{dataItem.OrderId}}\" ng-model='dataItem.ExpectedTimeOfDelivery' name=\"datepicker\" class=\"form-control\"></span>",
                ...
                //{
                //    field: "RequestDate", width: "175px", format: "{0:dd/MM/yyyy}", "title":
                //    $rootScope.resData.res_GridColumn_RequestedDate, type: "string", filterable: { mode: "row", extra: false }
                //},
                ...
                //{
                //    field: "ExpectedShift",
                //    template: "<select class=\"form-control\" style=\"margin:0px !important\"  ng-model=\"dataItem.ExpectedShift\" ng-change=\"getSelectedPickShiftvalue(dataItem.ExpectedShift,#=OrderId#)\" ng-disabled=\"RoleId === 3 || RoleId === 6\" ng-options=\"obj.LookUpId as obj.Name for obj in ShiftList\"><option value=\"\">Select</option></select>",
                ...
                {
                    field: "ExpectedShift", template: '<select class=\"form-control\" style=\"margin:0px !important\"  ng-model=\"dataItem.ExpectedShift\"  ng-disabled=\"RoleId === 3 || RoleId === 6\" ng-options=\"obj.LookUpId as obj.Name for obj in ShiftList\"><option value=\"\">Select</option></select>',
                ...
                //{
                //    field: "TruckInPlateNumber",
                //    template: "<input type=\"text\" ng-if=\"dataItem.Field1=='SCO'\" ng-model=\"dataItem.TruckInPlateNumber\" ng-readonly=\"dataItem.IsTruckInDisabled\"  />",
                ...
                //{
                //    field: "TruckOutPlateNumber",
                //    template: "<input type=\"text\" ng-if=\"dataItem.Field1=='SCO'\" ng-model=\"dataItem.TruckOutPlateNumber\" ng-readonly=\"dataItem.IsTruckOutDisabled || !dataItem.IsTruckInDisabled\" />",
                ...
                { field: "Save", title: "Save", template: "#if(CurrentState == 3) {#<a class=\"greenbgfont approvebtn\" ng-click=\"UpdateDeliveryPersonnelAndPlateNumber('#=SalesOrderNumber#')\"> Save </a>#} else {#<a class=\"greenbgfont approvebtn\" ng-click=\"UpdateDeliveryPersonnelAndPlateNumber('#=SalesOrderNumber#')\">  Update </a>#}#", type: "string", filterable: false, width: "120px" },
            ],
    ...
    $scope.GetAllDriverPlateNumberUpdated = function () {
        ...
        if (enquiryDetails.length > 0) {
            ...
            var columnNameDeliveryPersonnelId = 'DeliveryPersonnelId';
            for (var index = 0; index < gridDataArray.length; index++) {
                //var columnValue = gridDataArray[index][columnName];
                var columnValuePlateNumber = gridDataArray[index][columnName];
                var columnValueDeliveryPersonnelId = gridDataArray[index][columnNameDeliveryPersonnelId];
                var enquiryRequestDate = enquiryDetails.filter(function (el) { return el.OrderId === gridDataArray[index]["OrderId"]; });
                if (enquiryRequestDate.length > 0) {
                    //enquiryRequestDate[0].ExpectedTimeOfDelivery = columnValue;
                    enquiryRequestDate[0].PlateNumber = columnValuePlateNumber;
                    enquiryRequestDate[0].DeliveryPersonnelId = columnValueDeliveryPersonnelId;
                    enquiryRequestDate[0].CarrierId = $rootScope.UserId;
                    enquiryRequestDate[0].LocationType = 1;
                    enquiryRequestDate[0].UserId = $rootScope.UserId;
                    enquiryRequestDate[0].PlateNumberBy = "Carrier";
                }
            };
    ...
    $scope.getSelectedTransporatorvalue = function (id, orderId) {

        $scope.UpdateName = "BranchPlant";
        $scope.UpdateOrderId = orderId;
        $scope.TransporterId = id;
        //$scope.OpenReasoncodepopup(orderId, "BranchPlant");
        $scope.UpdateTransporter();
    ...
    $scope.UpdateTransporter = function () {
        ...
        var requestData =
            {
                ServicesAction: 'UpdateTransporter',
        ...
        GrRequestService.ProcessRequest(consolidateApiParamater).then(function (response) {
            ...
            $rootScope.ValidationErrorAlert('Carrier updated successfully.', '', 3000);
    ...
    $scope.UpdateBranchPlantCode = function () {
        ...
        GrRequestService.ProcessRequest(consolidateApiParamater).then(function (response) {
            ...
            $rootScope.ValidationErrorAlert('Collection location updated successfully.', '', 3000);
    ...
    $scope.SaveReasonCode = function () {
        ...
        if ($scope.ReasonCodeJson.ReasonCode !== "") {
            ...
            GrRequestService.ProcessRequest(consolidateApiParamater).then(function (response) {
                ...
                else if ($scope.UpdateName === "UpdateDriverAndPlateNumber") {
                    $scope.UpdatePickingDate();
                }
                else {
    ...
    $scope.UpdateDeliveryPersonnelAndPlateNumber = function (sonumber) {
        
        $scope.UpdateName = "UpdateDriverAndPlateNumber";
        $scope.UpdateSoNumber = sonumber;
        $scope.OpenReasoncodepopup(sonumber, "UpdateDriverAndPlateNumber");
    ...
    $scope.SaveDriverForSelectedEnquiry = function () {
        ...
        if (enquiryDetails.length > 0) {
            for (var i = 0; i < enquiryDetails.length; i++) {
                ...
                enquiryDetails[i].PlateNumberBy = "Carrier";
            }
    ...
    $scope.UpdatePickingDate = function () {
        ...
        var IsDeliveryPerssonnelcheck = false;
        var wareHouseDetails = $scope.GridData.data.filter(function (el) { return el.SalesOrderNumber === $scope.UpdateSoNumber; });
        ...
        var columnNameRevisedPlateNumber = 'PlateNumber';
        ...
        for (var index = 0; index < gridDataArray.length; index++) {
            var DeliveryPersonnelIdValue = gridDataArray[index][DeliveryPersonnelId];
            var columnValuecolumnNameRevisedPlateNumber = gridDataArray[index][columnNameRevisedPlateNumber];
            var sonumberGrid = wareHouseDetails.filter(function (el) { return el.SalesOrderNumber === gridDataArray[index]["SalesOrderNumber"]; });
            if (sonumberGrid.length > 0) {
                if (DeliveryPersonnelIdValue == 0 || DeliveryPersonnelIdValue == null) {
                    IsDeliveryPerssonnelcheck = true;
                }
                ...
                sonumberGrid[0].LocationType = 1;
                sonumberGrid[0].UserId = $rootScope.UserId;
                sonumberGrid[0].PlateNumberBy = "WareHouse";
                if (columnValuecolumnNameRevisedPlateNumber !== undefined) {
                    sonumberGrid[0].PlateNumber = columnValuecolumnNameRevisedPlateNumber;
                }
            }
    ...
    $scope.GetDriverAccToPlateNumber = function (plateNo, OrderId) {
        
        $rootScope.Throbber.Visible = true;
        var requestData =
            {
                ServicesAction: 'GetDriverAccToPlateNumberList',
                PlateNumber: plateNo
            };
        ...
        GrRequestService.ProcessRequest(jsonobject).then(function (response) {
            ...
            if (responseStr != undefined) {
                var dataList = $scope.GridData.data.filter(function (el) { return parseInt(el.OrderId) === parseInt(OrderId); });
                dataList[0].DeliveryPersonnelId = responseStr.TransporterVehicleList[0].DeliveryPersonnelId;
                //$scope.LoadDriverAndPlateNumber();
                $scope.SelectedDeliveryPersonnelId = responseStr.TransporterVehicleList[0].DeliveryPersonnelId;


            }
            else {
                $scope.SelectedDeliveryPersonnelId = "";
                $scope.SelectedPlateNumber = "";

            }
            $scope.SelectedOrderId = OrderId;
            $scope.SelectedPlateNumber = plateNo;
            gridCallBack();
        });
    }
