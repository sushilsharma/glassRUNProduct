angular.module("glassRUNProduct").controller('OrderListController', function ($scope, $q, $timeout, $rootScope, $document, $ionicModal, $filter, $location, $sessionStorage, $state, pluginsService, focus, GrRequestService, ManageOrderService) {
	...
	$scope.toggle_PartialRecieve_sidebar = function () {
		if ($('#Parial_Order_Receive_Panel').hasClass('open')) $('#Parial_Order_Receive_Panel').removeClass('open');
		else $('#Parial_Order_Receive_Panel').addClass('open');
	};



	$scope.toggle_OrderDetail_sidebar = function () {
		if ($('#Order_Details_Panel').hasClass('open')) $('#Order_Details_Panel').removeClass('open');
		else $('#Order_Details_Panel').addClass('open');
	};
	...
	//alert($scope.ViewControllerName);
	...
	$scope.AllowActionTime = 0;
	$scope.IsAllowSellerAction = false;
	$scope.LoadAllowSellerOrderReceiveOptionTime = function () {

		if ($sessionStorage.AllSettingMasterData !== undefined) {
			var allowSellerOrderReceive = $sessionStorage.AllSettingMasterData.filter(function (el) { return el.SettingParameter === 'IsAllowSellerToReceiveOrRejectOrder'; });
			if (allowSellerOrderReceive.length > 0) {
				if (allowSellerOrderReceive[0].SettingValue === "1") {
					$scope.IsAllowSellerAction = true;
				} else {
					$scope.IsAllowSellerAction = false;
				}
			...
			var sellerOrderReceiveTime = $sessionStorage.AllSettingMasterData.filter(function (el) { return el.SettingParameter === 'AccessTimeForSellerToReceiveOrRejectOrder'; });
			if (sellerOrderReceiveTime.length > 0) {
				$scope.AllowActionTime = parseInt(sellerOrderReceiveTime[0].SettingValue);
			}
		}
	};

	$scope.LoadAllowSellerOrderReceiveOptionTime();
	...
	$scope.LoadGridConfigurationData = function () {
		...
		return $q.all([
			...
			var ExternalSystemStatusList = $scope.ExtStatusResourcesList;

			var ExternalSystemStatusTextData = $scope.DynamicColumnList.filter(function (el) { return el.dataField == 'ExternalSystemStatusText' });

			if (ExternalSystemStatusTextData.length > 0) {

				ExternalSystemStatusTextData[0].lookup = {
					dataSource: ExternalSystemStatusList,
					displayExpr: "ResourceValue",
					valueExpr: "ResourceValue"
				};
		...
		var SalesAdminApprovalData = new DevExpress.data.CustomStore({
			load: function (loadOptions) {
				if (loadOptions.take !== undefined && loadOptions.take !== null) {
					...
					var DataResponseForLoadOrderListGrid = ManageOrderService.LoadOrderDetails(orderSearchDTO);
						...
						if (ResponseForLoadOrderListGrid.data != undefined) {
							if (ResponseForLoadOrderListGrid.data != undefined) {
								...
								angular.forEach($scope.SalesAdminApprovalList, function (item) {
									item.CollectionLocationCodeForCheck = item.BranchPlantCode;
									...
									if ($scope.IsAllowSellerAction === true) {
										if (item.OrderShippedDate !== undefined && parseInt(item.CurrentState) === parseInt("9056")) {
											if (item.OrderShippedDate !== null && item.OrderShippedDate !== "") {

												var shippeddate = item.OrderShippedDate;
									...
									item.OrderShippedConfirmOverDue = isOrderShippedOverDue;
									...
									if (item.IsEditPromiseDatePickUpDate === true) {
						...
						if (detailViewAvailable[0].IsDetailsViewAvailable === "1") {
							...
							if (e.column.caption === $rootScope.resData.res_GridColumn_SalesOrderNumber) {
								...
								if (page === "ControlTower") {
									...
									$scope.OrderDetailsData(e.data, e.data.OrderNumber);
								}
								else {
	...
	$scope.CreateWhereExpressionForGrid = function (loadOptions) {
		...
		var ExternalSystemStatus = "";
		var ExternalSystemStatusCriteria = "";
		...
		var CollectionStartTimeCriteria = "";
		var CollectionStartTime = "";

		var TruckOutDataTimeCriteria = "";
		var TruckOutDataTime = "";
		...
		var TruckInDataTimeCriteria = "";
		var TruckInDataTime = "";


		var ReceivedDateCriteria = "";
		var ReceivedDate = "";

		var TruckInDataTimeEndDateCriteria = "";
		var TruckInDataTimeEndDate = "";


		var ReceivedEndDateCriteria = "";
		var ReceivedEndDate = "";
		...
		var RelatedLoadNumberCriteria = "";
		var RelatedLoadNumber = "";
		debugger;
		...
		if (filterOptions != "") {
			...
			for (var i = 0; i < fields.length; i++) {
				...
				if (columnsList.includes("or") && fields[i].includes("Status")) {
				...
				else if (columnsList.includes("or") && (fields[i].includes("TruckSize") || fields[i].includes("Area") || fields[i].includes("Region") || fields[i].includes("ProvinceDesc") || fields[i].includes("ExternalSystemStatusText") || fields[i].includes("CollectionLocationCode") || fields[i].includes("BranchPlantCode") || fields[i].includes("CarrierCode"))) {
					...
					if (columnsList[0] === "ExternalSystemStatusText") {
						ExternalSystemStatusCriteria = columnsList[1];
						ExternalSystemStatus = columnsList[2];

						var ExternalSystemStatusParam = {};
						ExternalSystemStatusParam.fieldName = "ExternalSystemStatusText";
						ExternalSystemStatusParam.operatorName = ExternalSystemStatusCriteria;
						ExternalSystemStatusParam.fieldValue = ExternalSystemStatus;
						ExternalSystemStatusParam.fieldType = "string";
						OrderSearchParameterList.push(ExternalSystemStatusParam);
					...
					if (columnsList[0] === "ReceivedDate") {
						if (ReceivedDate === "") {
							ReceivedDateCriteria = columnsList[1];
							ReceivedDate = columnsList[2];
							ReceivedDate = new Date(ReceivedDate);
							ReceivedDate = $filter('date')(ReceivedDate, "dd/MM/yyyy");

							var ReceivedDateParam = {};
							ReceivedDateParam.fieldName = "ReceivedDate";
							ReceivedDateParam.operatorName = ReceivedDateCriteria;
							ReceivedDateParam.fieldValue = ReceivedDate;
							ReceivedDateParam.fieldType = "date";
							OrderSearchParameterList.push(ReceivedDateParam);

						} else {
							ReceivedEndDateCriteria = columnsList[1];
							ReceivedEndDate = columnsList[2];
							ReceivedEndDate = new Date(ReceivedEndDate);
							ReceivedEndDate = $filter('date')(ReceivedEndDate, "dd/MM/yyyy");

							var ReceivedEndDateParam = {};
							ReceivedEndDateParam.fieldName = "ReceivedDate";
							ReceivedEndDateParam.operatorName = ReceivedEndDateCriteria;
							ReceivedEndDateParam.fieldValue = ReceivedEndDate;
							ReceivedEndDateParam.fieldType = "date";
							OrderSearchParameterList.push(ReceivedEndDateParam);

						}
					}


					if (columnsList[0] === "TruckInDataTime") {
						if (TruckInDataTime === "") {
							TruckInDataTimeCriteria = columnsList[1];
							TruckInDataTime = columnsList[2];
							TruckInDataTime = new Date(TruckInDataTime);
							TruckInDataTime = $filter('date')(TruckInDataTime, "dd/MM/yyyy");

							var TruckInDataTimeParam = {};
							TruckInDataTimeParam.fieldName = "TruckInDataTime";
							TruckInDataTimeParam.operatorName = TruckInDataTimeCriteria;
							TruckInDataTimeParam.fieldValue = TruckInDataTime;
							TruckInDataTimeParam.fieldType = "date";
							OrderSearchParameterList.push(TruckInDataTimeParam);

						} else {
							TruckInDataTimeEndDateCriteria = columnsList[1];
							TruckInDataTimeEndDate = columnsList[2];
							TruckInDataTimeEndDate = new Date(TruckInDataTimeEndDate);
							TruckInDataTimeEndDate = $filter('date')(TruckInDataTimeEndDate, "dd/MM/yyyy");

							var TruckInDataTimeEndDateParam = {};
							TruckInDataTimeEndDateParam.fieldName = "TruckInDataTime";
							TruckInDataTimeEndDateParam.operatorName = TruckInDataTimeEndDateCriteria;
							TruckInDataTimeEndDateParam.fieldValue = TruckInDataTimeEndDate;
							TruckInDataTimeEndDateParam.fieldType = "date";
							OrderSearchParameterList.push(TruckInDataTimeEndDateParam);
					...
					if (columnsList[0] === "CollectionStartTime") {
						if (CollectionStartTime === "") {
							if (fields.length > 1) {
								CollectionStartTimeCriteria = "=";
							} else {
								CollectionStartTimeCriteria = columnsList[1];
							}
							CollectionStartTime = columnsList[2];
							CollectionStartTime = new Date(CollectionStartTime);
							CollectionStartTime = $filter('date')(CollectionStartTime, "dd/MM/yyyy HH:mm");

							var CollectionStartTimeParam = {};
							CollectionStartTimeParam.fieldName = "CollectionStartTime";
							CollectionStartTimeParam.operatorName = CollectionStartTimeCriteria;
							CollectionStartTimeParam.fieldValue = CollectionStartTime;
							CollectionStartTimeParam.fieldType = "date";
							OrderSearchParameterList.push(CollectionStartTimeParam);

						}
					}

					if (columnsList[0] === "TruckOutDataTime") {
						if (TruckOutDataTime === "") {
							if (fields.length > 1) {
								TruckOutDataTimeCriteria = "=";
							} else {
								TruckOutDataTimeCriteria = columnsList[1];
							}
							TruckOutDataTime = columnsList[2];
							TruckOutDataTime = new Date(TruckOutDataTime);
							TruckOutDataTime = $filter('date')(TruckOutDataTime, "dd/MM/yyyy HH:mm");

							var TruckOutDataTimeParam = {};
							TruckOutDataTimeParam.fieldName = "TruckOutDataTime";
							TruckOutDataTimeParam.operatorName = TruckOutDataTimeCriteria;
							TruckOutDataTimeParam.fieldValue = TruckOutDataTime;
							TruckOutDataTimeParam.fieldType = "date";
							OrderSearchParameterList.push(TruckOutDataTimeParam);
					...
					if (columnsList[0] === "Field5") {
						...
						var Field5Param = {};
						Field5Param.fieldName = "o.Field5";
						Field5Param.operatorName = Field5Criteria;
						Field5Param.fieldValue = Field5;
						Field5Param.fieldType = "string";
						OrderSearchParameterList.push(Field5Param);
					...
					if (columnsList[0] === "RelatedOrderNumber") {
						RelatedOrderNumberCriteria = columnsList[1];
						RelatedOrderNumber = columnsList[2];

						var RelatedOrderNumberParam = {};
						RelatedOrderNumberParam.fieldName = "RelatedOrderNumber";
						RelatedOrderNumberParam.operatorName = RelatedOrderNumberCriteria;
						RelatedOrderNumberParam.fieldValue = RelatedOrderNumber;
						RelatedOrderNumberParam.fieldType = "string";
						OrderSearchParameterList.push(RelatedOrderNumberParam);
					...
					if (columnsList[0] === "ProvinceDesc") {
						...
						OrderSearchParameterList.push(ProvinceDescParam);
					}
					...
					if (columnsList[0] === "Region") {
						...
						OrderSearchParameterList.push(RegionParam);
					}
					...
					if (columnsList[0] === "SubChannel") {
						...
						OrderSearchParameterList.push(SubChannelParam);
					}
					...
					if (columnsList[0] === "Priority") {
						PriorityCriteria = columnsList[1];
						if (columnsList[2] === '1') {
							Priority = 1;
						}
						...
						AvailableStockParam.operatorName = PriorityCriteria;
						AvailableStockParam.fieldValue = Priority;
						AvailableStockParam.fieldType = "string";
						OrderSearchParameterList.push(AvailableStockParam);
					}
					...
					if (columnsList[0] === "RelatedLoadNumber") {
						RelatedLoadNumberCriteria = columnsList[1];
						RelatedLoadNumber = columnsList[2];

						var RelatedLoadNumberParam = {};
						RelatedLoadNumberParam.fieldName = "RelatedLoadNumber";
						RelatedLoadNumberParam.operatorName = RelatedLoadNumberCriteria;
						RelatedLoadNumberParam.fieldValue = RelatedLoadNumber;
						RelatedLoadNumberParam.fieldType = "string";
						OrderSearchParameterList.push(RelatedLoadNumberParam);
		...
		if (loadOptions.sort !== null && loadOptions.sort !== undefined) {
			...
			else if (loadOptions.sort[0].selector === "Status") {
				OrderBy = ""
			}
			else {
		...
		var requestData =
		{
			...
			RelatedLoadNumber: RelatedLoadNumber,
			RelatedLoadNumberCriteria: RelatedLoadNumberCriteria,
...
debugger;
	...
	$scope.ExportToExcelSalesAdminApprovalDataold = function () {
	...
	$scope.ExportToExcelSalesAdminApprovalData = function () {
		...
		if ($scope.GridColumnList.length > 0) {
			...
			var orderSearchDTO = {
				...
				"PageControlName": PageControlName,
				"LoginId": parseInt($rootScope.UserId),
				"OrderSearchParameterList": $scope.OrderSearchParameterList,
				"whereClause": "",
				"IsExportToExcel": true,
				"OrderGridColumnList": $scope.RequestDataFilter.ColumnList,
			};



			ManageOrderService.ExportToExcelForOrderList(orderSearchDTO).then(function (response) {
	...
	$scope.ExportToExcelSalesAdminApprovalDataForRPMDetails = function () {
		...
		if ($scope.GridColumnList.length > 0) {
			...
			ManageOrderService.ExportToExcelForRPMOrderListDetails(orderSearchDTO).then(function (response) {
				...
				if (response.data !== undefined) {
					...
					if (blob.size > 0) {
						var filName = "RPMOrderDetailList_" + getDate() + ".csv";
						saveAs(blob, filName);
						$rootScope.Throbber.Visible = false;
					} else {
						$rootScope.ValidationErrorAlert('Document not generated.', '', 3000);
	...
	$scope.UpdateOrderReceiveInfo = function (orderJson, OrderId) {
		$rootScope.Throbber.Visible = true;


		var orderDTO = {
			ObjectId: orderJson.OrderId,
			ObjectNumber: orderJson.OrderNumber,
			CreatedBy: $rootScope.UserId,
			...
			LoginId: $rootScope.UserId,
			RoleId: $rootScope.RoleId,
			CurrentState: orderJson.CurrentState,
			ReceivedDate: $scope.IsoFomateDate(),
			EnquiryAutoNumber: orderJson.EnquiryAutoNumber

		};
		ManageOrderService.UpdateOrderReceive(orderDTO).then(function (response) {
			$rootScope.Throbber.Visible = false;
	...
	$scope.UpdateOrderReceiveData = function (orderJson, OrderId) {
		debugger;
		if (orderJson.WorkFlowCode === "B2BAPPFlow") {
			$scope.UpdateOrderReceiveInfo(orderJson, OrderId);
		} else {
			$scope.ConfirmDelivery(OrderId);
		}
	};


	$scope.OpenOrderPartialReceive = function (orderJson, OrderId) {
		debugger;

		$rootScope.SelectedOrderId = OrderId;
		$rootScope.SelectedOrderNumber = orderJson.OrderNumber;
		$rootScope.SelectedOrderStatus = orderJson.CurrentState;
		$rootScope.SelectedCurrentState = orderJson.CurrentState;
		$rootScope.SelectedEnquiryAutoNumber = orderJson.EnquiryAutoNumber;
		$rootScope.SelectedOrderJson = orderJson;
		if (orderJson.OrderProductList !== undefined && orderJson.OrderProductList !== null) {
			for (var i = 0; i < orderJson.OrderProductList.length; i++) {
				orderJson.OrderProductList[i].ReceiveQuantity = orderJson.OrderProductList[i].ProductQuantity;
			}
		}

		$rootScope.SelectedOrderJson.ProductList = orderJson.OrderProductList;
		$rootScope.SelectedOrderJson.ShipToName = orderJson.DeliveryLocationName;
		$rootScope.IsPartialReceive = true;
		$rootScope.BackFromOrderSurveyPage = false;
		$rootScope.LoadProductCountAndQuantity();
	...
	$scope.OpenChangePromisedDatePopup = function () {
		...
		if (enquiryDetails.length > 0) {
			...
			for (var i = 0; i < enquiryDetails.length; i++) {
				...
				if (newdateformatevalue > pickupdate) {
					ordersNumber = enquiryDetails[i].OrderNumber + "," + ordersNumber;
				}
			}
			...
			if (ordersNumber === "") {
				...
				$rootScope.ValidationErrorAlert(String.format($rootScope.resData.res_OrderList_PickupDateValidate, ordersNumber), 'error', 8000);
			}
	...
	$scope.OpenDeliveryDatePicker = function (e, item) {
		...
		if (newdateformatevalue > pickupdate) {
			...
			var splitdate = item.ExpectedTimeOfDelivery.split("T");
	...
	$scope.LoadData = function (Id) {
		...
		GrRequestService.ProcessRequest(consolidateApiParamater).then(function (response) {
			...
			if (response.data !== undefined) {
				...
				dataList[0].BranchPlantCode = response.data.Json.OrderList.CollectionLocationCode;
				dataList[0].BranchPlantName = response.data.Json.OrderList.CollectionLocationName;
	...
	$scope.GetRPMOrderDetailByOrderId = function (orderId) {
		...
		return $q.all([
			...
			$scope.CustomerConfirmMisMatchHeadeValueCount = ConfirmCountCheckOrderListGridresponse.data.Json.OrderList[0].CustomerConfirmMisMatchHeadeValue;
			...
			for (var i = 0; i < ConfirmCountCheckOrderListGridresponse.data.Json.OrderList[0].ProductInfo.length; i++) {
				...
				ConfirmCountCheckOrderListGridresponse.data.Json.OrderList[0].ProductInfo[i].TransporterQuantity = parseInt(ConfirmCountCheckOrderListGridresponse.data.Json.OrderList[0].ProductInfo[i].VerifiedQuantity);
				ConfirmCountCheckOrderListGridresponse.data.Json.OrderList[0].ProductInfo[i].CustomerQuantity = parseInt(ConfirmCountCheckOrderListGridresponse.data.Json.OrderList[0].ProductInfo[i].VerifiedQuantity);
				ConfirmCountCheckOrderListGridresponse.data.Json.OrderList[0].ProductInfo[i].CustomerQuantityValue = parseInt(ConfirmCountCheckOrderListGridresponse.data.Json.OrderList[0].ProductInfo[i].CustomerQuantityValue);
				ConfirmCountCheckOrderListGridresponse.data.Json.OrderList[0].ProductInfo[i].TransporterQuantityValue = parseInt(ConfirmCountCheckOrderListGridresponse.data.Json.OrderList[0].ProductInfo[i].TransporterQuantityValue);
			...
			$scope.IsTransporterConfirmHeaderValue = $scope.RPMOrderProductInfo[0].IsTransporterConfirm;
			$scope.IsCustomerConfirmHeaderValue = $scope.RPMOrderProductInfo[0].IsCustomerConfirm;
			$scope.IsCheckerConfirmHeaderValue = $scope.RPMOrderProductInfo[0].IsCheckerConfirm;
			...
			if (orderDetails[0].CurrentState == '10301') {
				if ($rootScope.RoleName == 'Customer') {
					if (orderDetails[0].CustomerQuantity === 0 && orderDetails[0].TransporterQuantity === 0 && orderDetails[0].ReasonCodeValue == "02") {
					...
					if (orderDetails[0].TransporterQuantity === 0 && orderDetails[0].CustomerQuantity === 0 && orderDetails[0].ReasonCodeValue == "01" && orderDetails[0].CustomerConfirmMisMatchValue == false) {
						$scope.ShowConfirmCountpopup();
					} else if (orderDetails[0].TransporterQuantity === 0 && orderDetails[0].CustomerQuantity !== 0 && orderDetails[0].ReasonCodeValue == "02" && orderDetails[0].CustomerConfirmMisMatchValue == true) {
						$scope.ShowConfirmCountpopup();
					}
					else {
						$scope.ShowiewConfirmCountpopup();
					...
					if (orderDetails[0].CurrentState == '10301' && (orderDetails[0].CustomerQuantity !== 0 || orderDetails[0].TransporterQuantity !== 0) && orderDetails[0].CustomerConfirmMisMatchValue == false) {
						$scope.ShowConfirmCountpopup();
					} else if (orderDetails[0].CurrentState == '10301' && (orderDetails[0].CustomerQuantity !== 0 && orderDetails[0].TransporterQuantity !== 0) && orderDetails[0].CustomerConfirmMisMatchValue == true) {
						$scope.ShowConfirmCountpopup();
					}
					else {
						$scope.ShowiewConfirmCountpopup();
	...
	$scope.ViewRPMOrderDetailByOrderId = function (orderId) {
		...
		return $q.all([
			...
			var ConfirmCountCheckOrderListGridresponse = resp[0];
			debugger;
			if (ConfirmCountCheckOrderListGridresponse.data.Json != null && ConfirmCountCheckOrderListGridresponse.data.Json != undefined) {
				if (ConfirmCountCheckOrderListGridresponse.data.Json.OrderList.length > 0) {
					$scope.RPMOrderInfo = ConfirmCountCheckOrderListGridresponse.data.Json.OrderList[0];
					$scope.RIOrderNumber = ConfirmCountCheckOrderListGridresponse.data.Json.OrderList[0].OrderNumber;
					$scope.ShipToCodeConfirmCount = ConfirmCountCheckOrderListGridresponse.data.Json.OrderList[0].SoldToCode;
					$scope.ShipToNameConfirmCount = ConfirmCountCheckOrderListGridresponse.data.Json.OrderList[0].SoldToName;
					...
					$scope.IsCustomerConfirmHeaderValue = $scope.RPMOrderProductInfo[0].IsCustomerConfirm;
					$scope.IsCheckerConfirmHeaderValue = $scope.RPMOrderProductInfo[0].IsCheckerConfirm;
					$scope.ShowViewItemDetailCountpopup();
	...
	$scope.ClearVaraibles = function () {
		$scope.RPMOrderInfo = "";
		$scope.RIOrderNumber = "";
		$scope.ShipToCodeConfirmCount = "";
		$scope.ShipToNameConfirmCount = "";
		$scope.CollectionLocationNameConfirmCount = "";
		$scope.CustomerConfirmMisMatchHeadeValueCount = "";
		$scope.TruckSizeConfirmCount = "";
		$scope.IsTransporterConfirmHeaderValue = "";
		$scope.IsCustomerConfirmHeaderValue = "";
		$scope.IsCheckerConfirmHeaderValue = "";
	}
	...
	$scope.SaveConfirmCount = function () {
		...
		$rootScope.Throbber.Visible = false;
		if ($rootScope.RoleName == 'Customer') {
			for (var i = 0; i < $scope.RPMOrderInfo.ProductInfo.length; i++) {
				if ($scope.RPMOrderInfo.ProductInfo[i].VerifiedQuantity !== parseInt($scope.RPMOrderInfo.ProductInfo[i].CustomerQuantity)) {
					$scope.RPMOrderInfo.ProductInfo[i].IsCustomerConfirmMisMatch = true;
				} else {
					$scope.RPMOrderInfo.ProductInfo[i].IsCustomerConfirmMisMatch = false;
				}
			}
	...
	$ionicModal.fromTemplateUrl('templates/ViewItemDetailCount.html', {
		scope: $scope,
		animation: 'fade-in-scale',
		backdropClickToClose: false,
		hardwareBackButtonClose: false
		...
		$scope.ViewItemDetailCountpopup = modal;
	});
	$scope.ShowViewItemDetailCountpopup = function () {
		//$scope.OrderIdConfirmCount = OrderId;

		$scope.ViewItemDetailCountpopup.show();
	}
	$scope.CloseViewItemDetailCountpopup = function () {

		$scope.ViewItemDetailCountpopup.hide();
	}
	...
	$scope.onChangeClick = function (e, item) {
		...
		GrRequestService.ProcessRequest(jsonobject).then(function (response) {
			...
			item.BranchPlantCode = item.CollectionLocationCodeForCheck;
	...
	$scope.SaveBranchPlant = function (e, item) {
		...
		$rootScope.LoadReasonCode("ChangeBranchPlantReasonForRPM");
		var isBranchPlantChange = false;
		$rootScope.Throbber.Visible = true;
		debugger;
		if (item.BranchPlantCode !== "" && item.BranchPlantCode !== undefined && item.BranchPlantCode !== null) {

			$scope.SelectedBranchPlantTemp = item.BranchPlantCode;
			$scope.OrderDetailsTemp = item;
			$scope.ScopeEvent = e;
	...
	$scope.SaveBranchPlantForParticularRowEnquiry = function () {
		...
		if ($scope.SelectedBranchPlantTemp !== "" && $scope.SelectedBranchPlantTemp !== undefined && $scope.SelectedBranchPlantTemp !== null) {
			...
			var object = {};
			object.ObjectId = $scope.OrderDetailsTemp.OrderId;
			...
			mainObject.ObjectList = objectList;
			mainObject.ObjectType = "Order";
			mainObject.ReasonCodeEventName = "UpdateBranchPlantForRPM";
			mainObject.FunctionName = "UpdateBranchPlantSelectedForParticularRowEnquiry";
			mainObject.FunctionParameter = $scope.OrderDetailsTemp;
	...
	$scope.UpdateBranchPlantSelectedForParticularRowEnquiry = function (orderDetails) {
		...
		var orderDTO = {
			...
			CollectionLocationCode: $scope.SelectedBranchPlantTemp,
			CreatedBy: $rootScope.UserId
		...
		ManageOrderService.UpdateBranchPlantForRPM(orderDTO).then(function (response) {
			$scope.CloseChangeBranchPlantCodeParticularRowPopup();
			$rootScope.Throbber.Visible = false;
			$scope.LoadData(selectedOrderId);
			$rootScope.ValidationErrorAlert(String.format($rootScope.resData.res_OMInquiryPage_BranchPlantUpdated), '', 3000);
	...
	$scope.OrderDetailsData = function (orderjson, orderNumber) {
		$rootScope.EntryFromPage = "app.ManageCustomerOrder";
		$rootScope.RedirectFromTrackInquiryOrderDetailsPage = 'app.ManageCustomerOrder';
		$rootScope.OrderDetailJson = orderjson;
		$rootScope.LoadOrderDetails(orderNumber);
	}
